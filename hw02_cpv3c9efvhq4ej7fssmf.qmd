...
monthindex <- seq(2,132, by = 2)
monthsequence <- c(start, start %m+% months(monthindex))
monthsequence[length(monthsequence)] <- monthsequence[length(monthsequence)] - days(1)
monthsequence
...
sample_intervals <- int_diff(sample_dates)
...
map(.x = num_lists, .f = sd, na.rm = TRUE)
